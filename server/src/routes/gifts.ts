...
function handleGiftError(error: unknown, res: Response) {
  if (error instanceof Error) {
    ...
    if (
      error.message.includes("jÃ¡ foi escolhido") ||
    ...
    if (
      ...
      return res.status(500).json({
        message:
          "A estrutura do banco de presentes está desatualizada. Reinicie a aplicação após publicar o backend atualizado.",
...
function assertImageBytes(contentType: keyof typeof imageExtensions, buffer: Buffer) {
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        return res.status(500).json({
          message:
            "O diretório de imagens não tem permissão de escrita. Revise UPLOADS_DIR na Hostinger.",
          code: fsError.code
        });
      }
