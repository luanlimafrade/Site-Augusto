...
async function ensureGiftColumns() {
  const existingColumns = new Set(
    (await getTableColumns("gifts")).map((column) => column.columnName)
  );
  const requiredColumns = [
    ...
    },
    {
      name: "image_position",
      definition:
        "image_position VARCHAR(40) NOT NULL DEFAULT 'center center' AFTER image_fit"
    },
    {
      name: "price_cents",
      definition:
        "price_cents INT UNSIGNED NOT NULL DEFAULT 0 AFTER image_position"
    },
    {
    ...
    },
    {
      name: "created_at",
      definition:
        "created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER sold_at"
    },
    {
      name: "updated_at",
      definition:
  ...
  if (missingColumns.length > 0) {
    ...
    if (Number(countRows[0]?.total || 0) > 0) {
      throw new Error(
        `A tabela gifts possui dados e precisa de migração manual. Colunas ausentes: ${missingColumns
  ...
  const incompatibleExtraColumns = finalColumns.filter(
    (column) =>
      !requiredColumnNames.has(column.columnName) &&
  ...
  if (incompatibleExtraColumns.length > 0) {
    throw new Error(
      `A tabela gifts possui colunas obrigatórias incompatíveis: ${incompatibleExtraColumns
